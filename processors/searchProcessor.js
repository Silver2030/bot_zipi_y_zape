...
async function buscar(botQueue, chatId, searchText) {
    ...
    try {
                ...
                try {
                    ...
                    switch (tipo) {
                        case 'user':
                            ...
                            nombre = u?.username || "Usuario Desconocido";
                            url = `https://app.warera.io/user/${id}`;
                            break;
                        case 'country':
                            const c = await getCountryData(id);
                            nombre = c?.name || "País Desconocido";
                            url = `https://app.warera.io/country/${id}`;
                            break;
                        case 'mu':
                            const m = await getMUData(id);
                            nombre = m?.name || "MU Desconocida";
                            url = `https://app.warera.io/mu/${id}`;
                            break;
                        case 'region':
                            const r = await apiCall('region.getById', { regionId: id });
                            nombre = r?.name || "Región Desconocida";
        ...
        for (const cat of categorias) {
            if (searchData[cat.key]?.length) {
                cat.datos = await obtenerDatosConNombre(searchData[cat.key], cat.tipo);
            }
        }
        ...
        categorias.forEach(({ nombre, datos }) => {
            if (datos.length) {
                mensaje += `*${nombre}*\n`;
                datos.forEach(item => {
                    mensaje += `[${escapeMarkdownV2(item.nombre)}](${item.url})\n`;
                });
                mensaje += '\n';
            }
        });
